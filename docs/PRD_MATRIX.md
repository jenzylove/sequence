...
| --- | --- | --- | --- | --- |
| **Automatic Reactivity delivery.** A settlement invokes the vault with no human in the loop, across a whole dependent chain, and the trader pays nothing for it. | `SequenceVault.onEvent` is the handler; `SequenceSubscriptionManager` (`0x88a3b514…`) owns the subscriptions and holds the single 35 STT stake. One `register` call walks the vault's real armed/queued chain and creates an exact-market subscription for every link, so later steps stay automatic after the first advances. | Two dependent settlements two hours apart, one registration, nobody present: `10:00:01Z Triggered → Placed → StepArmed → ChainAdvanced`, then `12:00:02Z ExposureReleased → Triggered → Placed`. Both steps PLACED, **0 manual resolution calls**, trader held 3.71 STT throughout. `docs/CHAINED_REACTIVITY_LIVE.json`. | **PASS** | Delivery still depends on the oracle answering that market: OracleHub does not emit `AnswerDelivered` for every settling market, and which series it answers varies. A sequence watching an unanswered market advances through the permissionless recovery path instead. `docs/FINDINGS.md` §1c. |
