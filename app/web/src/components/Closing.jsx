    ...
    <>
      <section className="closing-shell">
        <div className="relative mx-auto max-w-[1280px] overflow-hidden px-7 py-24 text-center sm:px-12 lg:px-16 lg:py-32">
          ...
          <p className="mx-auto mt-6 max-w-[470px] text-[13px] leading-[1.75] text-[#6f6a74]">Build the path, test its limits, then arm only when the Sequence reads exactly the way you intended.</p>
          <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
            <button onClick={onBuild} className="soft-button bg-[#111014] px-6 py-3 text-white">Build a sequence</button>
    ...
    <footer className="site-footer">
      <div className="mx-auto grid max-w-[1280px] gap-12 px-7 py-14 sm:px-12 md:grid-cols-[1.4fr_1fr_1fr] lg:px-16">
        <div>
          ...
          <p className="mt-5 max-w-[280px] text-[10px] leading-5 text-[#8e8993]">Outcome-driven execution on Somnia Shannon. Bounded by the rules you set.</p>
        ...
        <FooterGroup title="Product"><button onClick={onBuild}>Builder</button><a href="#how-it-works">Live state</a><a href="#proof">Execution proof</a></FooterGroup>
        <FooterGroup title="Technical"><a href="https://github.com/jenzylove/sequence/blob/ui/codex-sequence/docs/VERIFIED.md" target="_blank" rel="noreferrer">Verified interfaces ↗</a><a href="https://github.com/jenzylove/sequence" target="_blank" rel="noreferrer">GitHub ↗</a><a href="#top">Back to top</a></FooterGroup>
      </div>
      <div className="mx-auto flex max-w-[1152px] flex-wrap justify-between gap-3 border-t border-[#2b2930] px-7 py-5 text-[8px] uppercase tracking-[.12em] text-[#6e6973] sm:px-12"><span>Sequence · 2026</span><span>Plan → settle → execute</span></div>
    </footer>
  );
}
