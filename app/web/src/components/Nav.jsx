...
export default function Nav({ connected, wallet, onWallet, onDashboard, onBuilder, onDetails }) {
  return (
    <header className="relative z-20">
      <div className="mx-auto flex max-w-[1280px] items-center justify-between px-7 py-7 sm:px-12 lg:px-16">
        <button onClick={onDashboard} className="group flex items-center gap-2.5" aria-label="Sequence home">
        ...
        <nav className="hidden items-center gap-9 text-[12px] font-semibold text-[#585561] md:flex">
          {connected ? (
            <>
              <button onClick={onDashboard} className="transition hover:text-black">Your desk</button>
              <button onClick={onBuilder} className="transition hover:text-black">Advanced builder</button>
              <button onClick={onDetails} className="transition hover:text-black">Onchain details</button>
            </>
          ) : (
            <>
              <a href="#build" className="transition hover:text-black">Try it</a>
